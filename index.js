...
function getFlavorByIndex(menu,flavor){
    console.log(menu[flavor]);
  }
...
function removeFlavorByName(menu,flavors){
    let local = menu.indexOf(flavors);
    menu.splice(local);
    console.log(originalFlavors);
...
function copy(oldMenu, tempMenu){
  tempMenu = oldMenu.slice();
  return tempMenu
}
newMenu = copy(originalFlavors,newMenu);
...
function filterByWord(menu, filterString){
  let filteredFlavors = [];
  var i;
  for (i = 0; i < menu.length; i++) {
    if (menu[i].includes(filterString)) {
      filteredFlavors.push(menu[i]);
    }
  }
